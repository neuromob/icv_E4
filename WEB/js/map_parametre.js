...
function initMap() {
    ...
    map = new google.maps.Map(document.getElementById('mapParam'), options);
...
function convertLngLat(markerLatitude, markerLongitude){
    ...
    geocoder.geocode({'latLng': location}, function (results, status) {
        ...
        console.log("add : "+add);
        document.getElementById('adresseParam').value = add;

        document.getElementById("latitudeParam").value = markerLatitude;
        document.getElementById("longitudeParam").value = markerLongitude;
        }
    });
}
...
function convertAddress() {
    ...
    var address = document.getElementById("adresseParam").value;
	geocoder = new google.maps.Geocoder();
    geocoder.geocode( { 'address': address}, function(results, status) {
      if (status == google.maps.GeocoderStatus.OK) {
        document.getElementById('adresseParam').value = convertLngLat(results[0].geometry.location.lat(),results[0].geometry.location.lng());
        document.getElementById("latitudeParam").value = results[0].geometry.location.lat();
        document.getElementById("longitudeParam").value = results[0].geometry.location.lng();
