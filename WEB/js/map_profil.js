...
function convertLngLat(markerLatitude, markerLongitude){
    ...
    geocoder.geocode({'latLng': location}, function (results, status) {
        ...
        document.getElementById("latitude_profil").value = markerLatitude;
        document.getElementById("longitude_profil").value = markerLongitude;
        }
    });
}
...
function convertAddress() {
    ...
    geocoder.geocode( { 'address': address}, function(results, status) {
      if (status == google.maps.GeocoderStatus.OK) {
        ...
        document.getElementById("latitude_profil").value = results[0].geometry.location.lat();
        document.getElementById("longitude_profil").value = results[0].geometry.location.lng();
